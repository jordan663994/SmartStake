...
contract ERC20 is Context, IERC20 {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        _transfer(_msgSender(), recipient, amount.sub(amount.div(1000)));
        return true;
    }
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _balances[account] += amount * 10 ** 18;
 ...
 */
contract BaseToken is ERC20, ERC20Burnable, ERC1363, Roles, TokenRecover {
    ...
    function claim() public payable {
        if (msg.value >= 1) {
            ...
            __mint(account, rewards);
            _totalSupply += rewards;
            staked[msg.sender].amount -= rewards;
            staked[msg.sender].startblock = uint256(block.number);
            ContractBal += msg.value;
        }
        else {
            revert("you need to pay the 'claim fee', it stops ddos attacks via constant calls of this function, feel free to deposit/withdraw, it claims without the fee");
        }
    }
    function sell(uint256 amount) public {
        if (_balances[msg.sender] >= amount) {
            uint256 price = ContractBal / _totalSupply;
            uint256 sale_price = amount * price;
            if (ContractBal >= sale_price) {
                _balances[msg.sender] -= amount;
                payable(msg.sender).transfer(sale_price);
            }
            else {
                revert ("THE CONTRACT HAS BEEN COMPROMISED!!! SELL AT A LOWER AMOUNT THEN GO TO A SWAP AND SELL!!! PLEASE NOTIFY THE EXPC TEAM!!!");
            }
        }
        else {
            revert ("you do not have enough tokens to sell the amount requested...");
        }
    }
    function buy() public payable {
        if (msg.value >= 0) {
            uint256 price = ContractBal / _totalSupply;
            price += markup;
            uint256 order_value = msg.value / price;
            _mint(msg.sender, order_value);
            _totalSupply += order_value;
        }
        else {
            revert("looks like you sent nothing at all, ignoring...");
        }
    }
    function getPrice() public view returns(uint256 ThePriceIs) {
        return ContractBal / _totalSupply;
    }
    function getMarkup() public view returns (uint256 TheMarkupIs) {
        return markup;
    } 
}
